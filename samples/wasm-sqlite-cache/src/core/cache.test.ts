...
describe('SQLite cache contracts', () => {
  ...
  test('namespace filenames retain the full digest after the old truncation boundary', async () => { const subtle = { digest: async (_algorithm: string, data: Uint8Array) => Uint8Array.from({ length: 32 }, () => data.reduce((sum, value) => sum + value, 0) % 256) } as unknown as SubtleCrypto; Object.assign(globalThis, { TextEncoder: class { encode(value: string): Uint8Array { return Uint8Array.from(Array.prototype.map.call(value, (character: string) => character.charCodeAt(0))); } } }); const first = await namespaceFilename(namespace('user-a'), subtle); const second = await namespaceFilename(namespace('user-b'), subtle); expect(first).toHaveLength(64); expect(second).toHaveLength(64); expect(first).not.toBe(second); });
