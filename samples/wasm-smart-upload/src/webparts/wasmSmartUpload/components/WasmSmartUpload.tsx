...
export default class WasmSmartUpload extends React.Component<IWasmSmartUploadProps, State> {
  ...
  public render(): React.ReactElement {
    ...
    return <section className={styles.root} aria-labelledby="smart-upload-title">
      <h2 id="smart-upload-title">{strings.title}</h2>
      <p>{strings.description}</p>
      <div className={styles.dropZone} onDragOver={this.onDragOver} onDrop={this.onDrop}>
        <label htmlFor="smart-upload-file">{strings.choose}</label>
      ...
      <div className={styles.toolbar} aria-label={strings.controls}>
        <label htmlFor="smart-upload-chunk-size">{strings.chunk}<select id="smart-upload-chunk-size" value={this.state.chunkSize} onChange={this.onChunkSize} disabled={busy}>
      ...
      {result && <div className={styles.results} aria-labelledby="smart-upload-results-title">
        <h3 id="smart-upload-results-title">{strings.result}</h3>
  ...
  private workerPrepare = (request: PrepareRequest): Promise<PrepareResult> => new Promise((resolve, reject) => {
    ...
    worker.onerror = event => { worker.terminate(); this.worker = undefined; this.nativePrepare(fallbackRequest, resolve, reject, event.message || strings.workerFailed); };
