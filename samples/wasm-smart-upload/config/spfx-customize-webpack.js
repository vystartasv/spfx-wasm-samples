...
module.exports = function customizeWebpack(webpackConfiguration) {
  ...
  configurations.forEach(config => {
    ...
    config.plugins.push({ apply(compiler) { compiler.hooks.thisCompilation.tap('smart-upload-worker-manifest-audit', compilation => { const stage = compiler.webpack.Compilation.PROCESS_ASSETS_STAGE_REPORT - 1; compilation.hooks.processAssets.tap({ name: 'smart-upload-worker-manifest-audit', stage }, () => { const componentRuntimeChunks = new Set(Array.from(compilation.entrypoints.values()).map(entrypoint => entrypoint.getRuntimeChunk())); for (const chunk of compilation.chunks) if (!componentRuntimeChunks.has(chunk)) for (const group of chunk.groupsIterable) if (typeof group.setRuntimeChunk === 'function' && group.getRuntimeChunk() === chunk) group.setRuntimeChunk(undefined); }); }); } });
  });
