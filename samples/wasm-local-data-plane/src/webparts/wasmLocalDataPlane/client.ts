...
export class DataPlaneClient {
  private readonly worker = createDataPlaneWorker();
  private sequence = 0;
  private pending = new Map<string, { resolve: (value: unknown) => void; reject: (error: Error) => void }>();
  constructor() { this.worker.onmessage = event => { const response = event.data as RpcResponse; const item = this.pending.get(response.id); if (!item) return; this.pending.delete(response.id); if (response.ok) item.resolve(response.result); else item.reject(Object.assign(new Error(response.error?.message || 'Worker request failed.'), response.error)); }; this.worker.onerror = event => { this.pending.forEach(item => item.reject(new Error(event.message || 'Data plane worker failed.'))); this.pending.clear(); }; }
  request<T>(method: string, payload?: unknown): Promise<T> { const id = `rpc-${++this.sequence}`; return new Promise<T>((resolve, reject) => { this.pending.set(id, { resolve: resolve as (value: unknown) => void, reject }); this.worker.postMessage({ version: CONTRACT_VERSION, id, method, payload }); }); }
