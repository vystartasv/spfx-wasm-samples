...
scope.onmessage = event => { const request = event.data; const usable = !!request && typeof request === 'object' && typeof request.id === 'string' ? request.id : undefined; if (!request || typeof request !== 'object' || request.version !== RPC_VERSION || typeof request.id !== 'string' || typeof request.method !== 'string') { if (usable) scope.postMessage({ version: RPC_VERSION, id: usable, ok: false, error: { code: 'BAD_REQUEST', message: 'Malformed or unsupported worker request.' } }); return; } if (request.method === 'cancel') { cancelled.add(request.id); return; } if (request.method !== 'hash') { fail(request, 'BAD_REQUEST', 'Unknown worker method.'); return; } if (request.files !== undefined && !Array.isArray(request.files)) { fail(request, 'BAD_REQUEST', 'Malformed hash payload.'); return; } hash(request).then(result => scope.postMessage({ version: RPC_VERSION, id: request.id, ok: true, result })).catch(error => fail(request, error?.code || 'WORKER_ERROR', error?.message || 'Worker hashing failed.')); };
