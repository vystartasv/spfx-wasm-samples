...
export class AnalyticsClient {
  ...
  private start(): Worker {
    ...
    this.worker = createDuckdbAnalyticsWorker();
    this.worker.onmessage = event => { const pending = this.pending.get(event.data.id); if (!pending) return; this.pending.delete(event.data.id); event.data.ok ? pending.resolve(event.data.result) : pending.reject(new Error(event.data.error)); };
    this.worker.onerror = event => { this.pending.forEach(pending => pending.reject(new Error(event.message || 'DuckDB worker failed.'))); this.pending.clear(); };
    return this.worker;
  }
  ...
  public terminate(): void { this.worker?.terminate(); this.worker = undefined; }
