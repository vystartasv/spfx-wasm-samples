...
module.exports = function customizeWebpack(webpackConfiguration) {
  ...
  configurations.forEach(config => {
    ...
    config.module.rules.unshift(
      { test: /\.wasm$/, type: 'asset/resource', generator: { filename: 'qr-assets/[name][ext]' } }
    );
    config.experiments = { ...config.experiments, asyncWebAssembly: false };
    config.plugins = config.plugins || [];
    config.plugins.push({
      apply(compiler) {
        compiler.hooks.thisCompilation.tap('qr-worker-manifest-audit', compilation => {
          const stage = compiler.webpack.Compilation.PROCESS_ASSETS_STAGE_REPORT - 1;
          compilation.hooks.processAssets.tap({ name: 'qr-worker-manifest-audit', stage }, () => {
